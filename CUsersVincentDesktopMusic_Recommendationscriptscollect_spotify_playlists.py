...
class SpotifyPlaylistCollector:
    ...
    async def collect_from_categories(self, n_playlists_per_category: int = 50):
        ...
        Categories: pop, rock, hip-hop, electronic, country, etc.
        """
        logger.info("Fetching Spotify categories...")

        try:
            # Get available categories
            response = await self.spotify._make_request(
                "GET",
                "browse/categories",
                params={"limit": 50}
            )
            ...
            for category in categories:
                ...
                try:
                    ...
                    cat_response = await self.spotify._make_request(
                        "GET",
                        f"browse/categories/{category_id}/playlists",
                        params={"limit": 50}
                    )

                    playlists = cat_response.get("playlists", {}).get("items", [])
    ...
    async def collect_from_user_playlists(self, user_ids: List[str]):
        ...
        Args:
            user_ids: List of Spotify user IDs with public playlists
        """
    ...
    def save_playlists(self, output_dir: str = "data/smpd/raw"):
        ...
        # Split into slices (1000 playlists per slice like SMPD)
...
async def main():
    ...
    # Strategy 3: Collect from known curators (optional)
    curators = [
        "spotify",
        "digster",
    ]
    ...
    logger.info("=" * 60)
    logger.info("\nNext steps:")
    logger.info("1. Run: py scripts/download_smpd.py")
    logger.info("2. Run: py scripts/train_item2vec.py")
